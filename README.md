...
* SyncHost: remote host you sync your mail tree to (deF: none)
* SyncCmd: command used to sync (def: rsync -azv)
* SyncRemoteDir: dir on SyncHost to sync your mail to (def: $HOME)
* MairixCmd: mairix(1) invocation to (re)index your mail tree
* MairixIndexOptrs: options to mairix(1)
* MBDefaultCmd: command to run when you enter a dir (def: minbox)
* ShellEscapeCmd: command to use for shell escapes, e.g. /bin/ksh -l
* MBReadLineCmd: path to rl
* MBHistoryFile: where to store your mb history from rl (~/.mb.history)
* MBHistoryMax: max entries in mb history (none)
* MBStartQuiet: bool true to skip opening message
...
A brief description of what these scripts do:
    ...
    plusfiler   file messages addressed to me+something@somewhere properly
                meant to be invoked from e.g. your ~/.fdm.conf, or whatever
                mechanism you use to fetch email
